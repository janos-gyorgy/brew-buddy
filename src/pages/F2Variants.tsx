...
import type { F2Variant } from "@/lib/types";
        ...
        {variants && variants.length > 0 ? (
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-lg text-foreground mb-1">{variant.name}</h3>
                    ...
                    {(variant.fruits_and_juices || variant.herbs_and_spices) && (
                      <div className="mt-3 pt-3 border-t border-border text-xs text-muted-foreground space-y-1">
                        {variant.fruits_and_juices && <p className="truncate">🍓 {variant.fruits_and_juices}</p>}
                        {variant.herbs_and_spices && <p className="truncate">🌿 {variant.herbs_and_spices}</p>}
                      </div>
                    )}
                  </CardContent>
                </Card>
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-sm text-muted-foreground mt-2">Create F2 variants from your batches</p>
            </CardContent>
          </Card>
        )}
      </div>

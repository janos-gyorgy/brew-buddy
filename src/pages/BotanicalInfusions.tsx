  ...
  const { data: infusions, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("botanical_infusions" as any)
        .select("*")
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Dried camomile flowers"
                    required
                  />
                </div>
                <div className="grid gap-4 grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <DefaultInput
                      ...
                      onValueChange={(v) => handleChange("amount_g", v)}
                      defaultFillValue="3"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <DefaultInput
                      ...
                      onChange={(e) => handleChange("water_ml", e.target.value)}
                      onValueChange={(v) => handleChange("water_ml", v)}
                  ...
                  <div className="space-y-2">
                    ...
                    <DefaultInput
                      ...
                      onValueChange={(v) => handleChange("temp_c", v)}
                      defaultFillValue="50"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <DefaultInput
                      id="steep_minutes"
                      type="number"
                      value={formData.steep_minutes}
                      onChange={(e) => handleChange("steep_minutes", e.target.value)}
                      onValueChange={(v) => handleChange("steep_minutes", v)}
                      defaultFillValue="10"
                    />
                  </div>
                </div>
              ...
              <Card key={infusion.id} className="hover:border-primary transition-colors">
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-3 text-sm text-muted-foreground mb-4">
                    ...
                    {infusion.steep_minutes && (
                      <span className="flex items-center gap-1 bg-muted px-2 py-1 rounded">
                        <Clock className="h-3 w-3" />
                        {infusion.steep_minutes} min
                      </span>
                    )}
                  </div>
                  {infusion.notes && (
                    <p className="text-sm text-muted-foreground line-clamp-2 mb-4">{infusion.notes}</p>
                  )}
